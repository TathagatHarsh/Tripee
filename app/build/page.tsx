 ...
 * carried in.
 *
 * This used to be a bare redirect to /build/shape, which meant every query
...
function BuildEntry() {
  ...
  useEffect(() => {
    ...
    if (slug) {
      ...
      if (preset) return load(preset.config, "/build/review");
    }
    ...
    if (Object.keys(patch).length) return load({ ...current, ...patch }, "/build/shape");
    router.replace("/build/shape");
    }
  }, [params, router]);
