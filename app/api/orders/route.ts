...
export async function POST(req: Request) {
  ...
  const [design, bakery] = await Promise.all([
    body.designSlug
      ? db.design.findUnique({ where: { slug: body.designSlug } })
      : null,
  ...
  try {
    ...
    if (error instanceof ReferenceUnavailable) {
      return Response.json(
        {
          error: "We couldn't allocate an order reference. Please try again.",
          code: "ref_exhausted",
        },
        { status: 503 },
      );
...
async function createOrder(
  ...
  return db.$transaction(
    async (tx) => {
      await tx.$queryRaw`SELECT 1 AS locked FROM pg_advisory_xact_lock(hashtext(${`checkout:${input.idempotencyKey}`}))`;
      ...
      await tx.$queryRaw`SELECT 1 AS locked FROM pg_advisory_xact_lock(hashtext(${`capacity:${capacityKey}`}))`;

      const blackoutDate = new Date(
        `${input.fulfillment.requestedDate}T00:00:00.000Z`,
      );
      ...
      if (blackout) {
        throw new CapacityUnavailable(
          blackout.reason || "The bakery is closed on that date.",
        );
      }

      const dayStart = new Date(
        `${input.fulfillment.requestedDate}T00:00:00+05:30`,
      );
      ...
      if (capacity > 0) {
        ...
        const requested = input.quotes.reduce(
          (sum, quote) => sum + quote.qty,
          0,
        );
      ...
      const order = await tx.order.create({
        ...
        data: {
          ...
          pincode:
            input.fulfillment.method === "delivery"
              ? input.fulfillment.pincode
              : null,
          deliverySlot: input.fulfillment.slot,
          leadHours: input.quotes[0].slot.effectiveLeadHours,
          requestedFor: input.requestedFor,
          requestedWindow: slotWindow(input.quotes[0].slot),
          ...
          deliveryZoneIdSnapshot:
            input.fulfillment.method === "delivery"
              ? input.quotes[0].slot.zoneId
              : null,
          deliveryZoneNameSnapshot:
            input.fulfillment.method === "delivery"
              ? input.quotes[0].slot.zoneName
              : null,
          allergens: [
            ...new Set(cakeRows.flatMap((cake) => cake.allergens)),
          ].sort(),
      ...
      const response = responseFor(order);
