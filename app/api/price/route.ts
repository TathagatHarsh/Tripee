...
import { CakeChoices, configForVariant, sellable, variantLabel } from "@/lib/cakes";
...
export async function POST(req: Request) {
  ...
  if (Array.isArray(b.items)) {
    ...
    if (!parsed.success) {
      return Response.json({ error: "That quote couldn't be read.", code: "invalid_request" }, { status: 400 });
    }
    ...
    if (!schedule.ok) {
      return Response.json({ error: schedule.message, code: "schedule_unavailable" }, { status: 422 });
    }
    const items = parsed.data.items.map((item) => item.cakeSlug
      ? {
          ...item,
          choices: {
            ...item.choices!,
            delivery: fulfillment.slot,
            pincode: fulfillment.method === "delivery" ? fulfillment.pincode : undefined,
          },
        }
      : item);
  ...
  if (typeof b.cakeSlug === "string") {
    ...
    if (!choices.success) {
      return Response.json({ error: "Invalid delivery choices" }, { status: 400 });
    }
    ...
    if (!cake) {
      return Response.json(
        { error: "That cake is no longer available.", code: "cake_unavailable" },
        { status: 409 },
      );
    }
