import { SignUp } from "@clerk/nextjs";
import type { Metadata } from "next";
import { AuthSheet, authClerkAppearance } from "@/components/AuthSheet";

/**
 * Creating an account is optional in this product and always has been — see the
 * line under the form. Nothing here grants a role: every account Clerk mints
 * arrives as a CUSTOMER, because lib/auth's `loadProfile` creates the row with
 * the column default and there is no field on this form, or anywhere in the
 * request, that could say otherwise.
 */

export const metadata: Metadata = {
  title: "Create an account — MakeYourCakes",
  robots: { index: false, follow: false },
};

export default function SignUpPage() {
  return (
    <AuthSheet
      title="Create your account"
      lede="Save the cakes you like and keep every order in one place. It takes about a minute."
    >
      <SignUp appearance={authClerkAppearance} />
    </AuthSheet>
  );
}
