...
export function StepFooter({ price }: { price?: React.ReactNode }) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft" && prev) router.push(`/build-legacy/${prev.slug}`);
      if (e.key === "ArrowRight" && next && !blocked) router.push(`/build-legacy/${next.slug}`);
    };
  ...
  return (
    <div className="shrink-0 border-t border-rule bg-slab px-3.5 pt-3.5 pb-[max(1.125rem,env(safe-area-inset-bottom))] lg:px-3.5">
      ...
      <div className="flex items-center gap-2.5">
        <Link
          href={prev ? `/build-legacy/${prev.slug}` : "/"}
        ...
        {next && (
          <Link
            href={blocked ? pathname : `/build-legacy/${next.slug}`}
